...
- For each selected area, dispatch the deep-dive investigation to a code-exploration sub-agent whenever one is available — even for a single area — seeding it with the area's existing findings doc (evidence map) so covered code is not re-scanned (see **multi-agent-orchestration.md**). Fall back to direct execution only when no suitable sub-agent exists.
