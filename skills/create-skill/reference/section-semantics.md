...
Used by **create-skill-file** (structure step 7) and **validate-created-skill** (structure step 1).
...
| `<context-loading-guide>` (in knowledge) | On-demand context router | Condition-first table (`Load when` \| `Provides` \| `File`) |
...
- Examples embedded inline instead of referenced by file path
- `<context-loading-guide>` as a bullet list, or description-first instead of condition-first
- Using `<knowledge>`/`<capabilities>`/`<rules>` tag syntax to reference sections in prose — use plain names
