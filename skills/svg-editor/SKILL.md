...
- **Label placement**: Place connection labels at the midpoint of the line, offset 8–12px perpendicular to the line direction. The label must not overlap any line segment — if a label sits near a bend, move it to the straight segment with more room. Use a white `<rect>` behind the label text to mask the line underneath.
