...
Apply when a file exceeds **both** the line and char budget (or the char budget alone).
The char budget is the binding constraint — reformatting cannot reduce chars, so the only real levers are redistribution and reduction.

## Priority order: redistribute → reduce → escalate

### Redistribute — move content, don't cut it (fixes the per-file budget)
1. **Extract to `reference/`** — Move large rubrics, tables, criteria, or API lists out of `<knowledge>` and out of capability steps into `reference/*.md`;
...
4. **Split into a new skill** — If the chunk has its own trigger scenarios, knowledge, and examples and does not need shared state,
...
5. **Dedupe** — Remove content restated elsewhere: rules re-stating capabilities, capability preambles re-describing delegated skills
   ("per X" instead of 5 lines), when-to-use re-stating the description, knowledge re-stating reference files.
6. **Merge related capabilities** — Collapse 2–3 small capabilities into one to remove objective/preamble boilerplate and repeated "load X" first-steps.
   Do not merge when the split isolates genuinely different procedures.
