...
- `api-gateway/F3` — **failed** (compile errors; reason recorded)
- `order-service/F4` — **unplanned** (wave 2, waits F2 + F3)
- `api-gateway/F4` — **unplanned** (wave 2, waits F2 + F3)
...
| api-gateway/F3 | **hold** | failed — ask user: re-plan (plan-development-task) or retry |
| order-service/F4 | **wait** | blocked-by F2 + F3 |
| api-gateway/F4 | **wait** | blocked-by F2 + F3 |
...
- **Agent C** (solution-doc-writer / adr-writer) → update solution-doc §Wallet and ADR-001 if F2's execution surfaces artifact changes — the orchestrator never edits them itself
...
Resumed F2 from step 4 · skipped F1 (done) · F3 needs a re-plan/retry decision · both F4 cells wait for F2 + F3. Completed work was never redone.
