...
<write-boundary>
Writes are confined to the spike folder (`**/spikes/**`). Never modify code, config, tests, or files outside it, and never build prototypes or proof-of-concept code (those belong to `orchestrate-feature-delivery`). After each write capability, apply the spike write-boundary check from the `conduct-spike` skill. Record implementation needs as an out-of-scope note in `scope.md` / `solution.md`; the user runs `orchestrate-feature-delivery` to touch code.
</write-boundary>
